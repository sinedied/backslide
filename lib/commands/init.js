...
function init(fromTemplateDir, force) {
  ...
  const starterPath = path.join(__dirname, '../..', StarterDir);
  fromTemplateDir = fromTemplateDir ? path.resolve(fromTemplateDir) : path.join(starterPath, TemplateDir);
  try {
    fs.copySync(fromTemplateDir, TemplateDir);
    fs.copySync(path.join(starterPath, 'presentation.md'), './presentation.md');
