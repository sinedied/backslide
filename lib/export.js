...
async function exportFile(dir, file, options) {
  ...
  [md, html, css] = await Promise.all([
    ...
    fs.readFile(path.join(TemplateDir, HtmlTemplate)).toString(),
    sass()
  ]);
  ...
  suppressErrorOutput();
  html = await inline ? inline(TemplateDir, html) : html;
...
async function inline(basedir, html) {
  const newHtml = await new Promise((resolve, reject) => {
